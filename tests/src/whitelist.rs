...
use crate::{
    clear_state, get_test_address, pre_test, switch_to_dfx_test_key1, switch_to_dfx_test_key2,
    sybil_execute,
};
...
fn test_whitelist_with_valid_data() {
    ...
    add_to_whitelist(&test_address).expect("failed to add to whitelist");
    assert_eq!(
        is_whitelisted(&test_address),
        Ok(true),
        "address is not whitelised"
    );

    remove_from_whitelist(&test_address).expect("failed to remove from whitelist");
    assert_eq!(
        is_whitelisted(&test_address),
        Ok(false),
        "address is whitelised"
    );

    add_to_whitelist(&test_address).expect("failed to add to whitelist");
    let whitelist = get_whitelist().expect("failed to get whitelist");
    assert!(
        whitelist.contains(&test_address),
        "address is not whitelised"
    );
}
...
fn test_whitelist_with_invalid_test_address_error() {
    ...
    assert!(
        add_to_whitelist(&invalid_test_address)
            .unwrap_err()
            .contains("invalid hex"),
        "should fail with invalid hex"
    );
    assert!(
        remove_from_whitelist(&invalid_test_address)
            .unwrap_err()
            .contains("invalid hex"),
        "should fail with invalid hex"
    );
    assert!(
        is_whitelisted(&invalid_test_address)
            .unwrap_err()
            .contains("invalid hex"),
        "should fail with invalid hex"
    );
...
fn test_whitelist_with_is_whitelisted_error() {
    ...
    assert!(
        add_to_whitelist(&test_address)
            .unwrap_err()
            .contains("Address is already whitelisted"),
        "should fail with user is already whitelisted"
    );
}
...
fn test_whitelist_with_not_whitelisted_error() {
    ...
    assert!(
        remove_from_whitelist(&test_address)
            .unwrap_err()
            .contains("Address is not whitelisted"),
        "should fail with user is not whitelisted"
    );
}
...
fn test_whitelist_not_controller_error() {
    ...
    assert!(
        add_to_whitelist(&test_address)
            .unwrap_err()
            .contains("Caller is not a controller"),
        "should fail with not controller"
    );
    assert!(
        remove_from_whitelist(&test_address)
            .unwrap_err()
