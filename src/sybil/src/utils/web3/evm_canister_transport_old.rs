...
fn map_chain_id_to_service(chain_id: u64) -> RpcServices {
    match chain_id {
...
impl Transport for EVMCanisterTransport {
    ...
    fn send(&self, _: RequestId, call: Call, _: CallOptions) -> Self::Out {
        ...
        let source = match self.rpcs_url {
            ...
            None => map_chain_id_to_service(self.chain_id),
        };
        ...
        match call {
            Call::MethodCall(method_call) => match method_call.method.as_str() {
                ...
                _ => Box::pin(async move {
                    execute_canister_call(ic_eth_rpc, source, json_rpc_payload, max_response_bytes)
                        .await
                }),
            },
            _ => Box::pin(async move {
